...
func EnsureInstalledWithForce(force bool) error {
	...
	if agentInstalled {
		fmt.Println("Installed Kiro agent config: ~/.kiro/agents/wetwire-azure-runner.json")
	}
	if mcpInstalled {
...
// ensureAgentConfig installs the wetwire-azure-runner agent to ~/.kiro/agents/
...
func ensureAgentConfig(force bool) (bool, error) {
	...
	agentPath := filepath.Join(agentDir, "wetwire-azure-runner.json")
...
// findMCPBinaryPath returns the path to wetwire-azure-mcp binary.
...
func findMCPBinaryPath() string {
	// Try to find wetwire-azure-mcp next to current executable
	exe, err := os.Executable()
	if err == nil {
		if resolved, err := filepath.EvalSymlinks(exe); err == nil {
			exe = resolved
		}
		mcpPath := filepath.Join(filepath.Dir(exe), "wetwire-azure-mcp")
		if _, err := os.Stat(mcpPath); err == nil {
			return mcpPath
		}
	}

	// Check if wetwire-azure-mcp is in PATH
	if path, err := exec.LookPath("wetwire-azure-mcp"); err == nil {
		return path
	}
...
// Uses the standalone wetwire-azure-mcp binary.
...
func getMCPServerConfig() mcpServer {
	...
	return mcpServer{
		Command: "go",
		Args:    []string{"run", "github.com/lex00/wetwire-azure-go/cmd/wetwire-azure-mcp@latest"},
		Cwd:     cwd,
	}
}

// GetCoreConfig returns a corekiro.Config for the Azure domain.
